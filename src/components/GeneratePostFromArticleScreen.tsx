...
interface GeneratePostFromArticleScreenProps {
  onBack: () => void;
}

const GeneratePostFromArticleScreen: React.FC<GeneratePostFromArticleScreenProps> = ({ onBack }) => {
  const [selectedFormat, setSelectedFormat] = useState('post-formats');
  const [articleUrl, setArticleUrl] = useState('https://www.example.com/blog-post');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Generate a post from an article</h1>
          <p className="text-gray-600">Share a link to a blog post and generate a post from it</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        ...
        {/* Blog Post URL Section */}
        <div className="mb-8">
          <label className="block text-lg font-semibold text-gray-900 mb-4">
            Blog post URL (the URL content should be open and public)
          </label>
          <p className="text-gray-600 text-sm mb-4">
            Currently, we don't support PDFs, Google Docs.
          </p>
          <input
            type="url"
            value={articleUrl}
...
export default GeneratePostFromArticleScreen;